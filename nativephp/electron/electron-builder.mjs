...
function patchBluetoothUsageDescriptions(plistPath) {
...
function patchMacInfoPlists(context) {
    if (context.electronPlatformName !== 'darwin') {
        return;
    }

    const appContentsPath = join(
        context.appOutDir,
    ...
    const patchInfoPlistsRecursively = (directory) => {
        for (const file of readdirSync(directory)) {
            ...
            if (stats.isDirectory()) {
                patchInfoPlistsRecursively(path);

                continue;
            }

            if (file === 'Info.plist') {
                patchBluetoothUsageDescriptions(path);
            }
        }
    };

    if (existsSync(appContentsPath)) {
        patchInfoPlistsRecursively(appContentsPath);
    }
}
...
export default {
    ...
    mac: {
        ...
        extendInfo: {
            ...
            NSBluetoothPeripheralUsageDescription: bluetoothUsageDescription,
        },
    },
    dmg: {
        artifactName: appName + '-${version}-${arch}.${ext}',
