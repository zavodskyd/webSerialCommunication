<div class="min-h-screen bg-slate-100">
    <div class="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <a href="{{ route('votings.index') }}" wire:navigate
                ...
                <p class="mt-2 text-base text-slate-500">Uprav hlavičku, nadpis, logo, predvolené časy a priprav otázky
                    pre živé hlasovanie.</p>
            </div>
            ...
            <div class="rounded-[1.5rem] bg-white px-5 py-4 text-sm text-slate-600 shadow-sm ring-1 ring-slate-200">
                ...
                <a href="{{ route('votings.presentation', $voting) }}" target="_blank"
                    class="mt-3 inline-flex text-sm font-semibold text-sky-700 hover:text-sky-900">
                    Otvoriť prezentačné okno →
                </a>
                @if (count($questionRows) > 0)
                    <a href="{{ route('votings.console', $voting) }}"
                        class="mt-2 inline-flex text-sm font-semibold text-emerald-700 hover:text-emerald-900">
        ...
        <div class="grid gap-8 xl:grid-cols-[1.15fr,0.85fr]">
            <section class="rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-slate-200/80">
                <div class="flex items-center justify-between gap-3">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500">Základné nastavenia použiteľné pre projekciu aj
                            administráciu.</p>
                    </div>
                </div>
                ...
                <form wire:submit="saveVoting" class="mt-6 grid gap-5">
                    <div class="grid gap-5 md:grid-cols-2">
                        <div>
                            ...
                            <input type="text" wire:model.blur="name"
                                class="mt-2 w-full rounded-2xl border-slate-300 px-4 py-3 text-slate-900 focus:border-sky-500 focus:ring-sky-300">
                            @error('name')
                    ...
                    <div>
                        ...
                        <textarea wire:model.blur="title" rows="3"
                            class="mt-2 w-full rounded-2xl border-slate-300 px-4 py-3 text-slate-900 focus:border-sky-500 focus:ring-sky-300"></textarea>
                        @error('title')
                            <p class="mt-2 text-sm text-rose-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <textarea wire:model.blur="headerText" rows="3"
                            class="mt-2 w-full rounded-2xl border-slate-300 px-4 py-3 text-slate-900 focus:border-sky-500 focus:ring-sky-300"></textarea>
                        @error('headerText')
                            <p class="mt-2 text-sm text-rose-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="grid gap-5 md:grid-cols-2">
                        <div>
                            ...
                            <div
                                class="mt-2 flex items-center gap-3 rounded-2xl border border-slate-300 bg-white px-4 py-3">
                                <input type="number" min="5" max="600"
                                    wire:model.blur="defaultResponseTimeSeconds"
                                    class="w-full border-0 p-0 text-slate-900 focus:ring-0">
                                <span class="text-sm text-slate-500">sekúnd</span>
                            </div>
                            @error('defaultResponseTimeSeconds')
                                <p class="mt-2 text-sm text-rose-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div>
                            ...
                            <div class="mt-3 rounded-2xl border border-slate-200 bg-slate-50 p-3">
                                @if ($logoUpload)
                                    <img src="{{ $logoUpload->temporaryUrl() }}" alt="Náhľad loga"
                                        class="h-24 w-full rounded-xl object-contain bg-white">
                                @elseif ($logoPath)
                                    <img src="{{ route('votings.logo', $voting) }}" alt="Aktuálne logo"
                                        class="h-24 w-full rounded-xl object-contain bg-white">
                                @else
                                    <div
                                        class="flex h-24 items-center justify-center rounded-xl border border-dashed border-slate-300 bg-white text-sm text-slate-400">
                    ...
                    <label class="flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
                        <input type="checkbox" wire:model.live="autoShowResults"
                            class="mt-1 rounded border-slate-300 text-slate-900 focus:ring-slate-400">
                        <span>
                            <span class="block text-sm font-semibold text-slate-800">Automaticky zobraziť výsledok po
                                otázke</span>
                            <span class="mt-1 block text-sm text-slate-500">Ak je vypnuté, konzola po ukončení otázky
                                preskočí rovno na ďalšiu otázku.</span>
                        </span>
                    </label>
            ...
            <section
                class="rounded-[2rem] bg-[linear-gradient(160deg,_#eff6ff,_#ffffff_45%,_#ecfeff)] p-6 shadow-sm ring-1 ring-sky-100">
                <div>
                    ...
                    <p class="mt-2 text-sm leading-6 text-slate-600">Vygenerované otázky sa pridajú za už existujúce
                        poradie, takže vieš pripravovať schôdzu postupne.</p>
                </div>

                <form wire:submit="generateQuestions" class="mt-6 space-y-5">
                    <div>
                        <label class="text-sm font-medium text-slate-700">Názov otázok</label>
                        <input type="text" wire:model.blur="questionLabel"
                            class="mt-2 w-full rounded-2xl border-sky-200 bg-white/90 px-4 py-3 text-slate-900 focus:border-sky-500 focus:ring-sky-300">
                        <p class="mt-2 text-sm text-slate-500">Použije sa pri ručnom pridaní aj pri hromadnom generovaní
                            otázok.</p>
                        @error('questionLabel')
                            <p class="mt-2 text-sm text-rose-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="grid gap-5 md:grid-cols-2">
                        <div>
                            ...
                            <input type="number" min="1" max="200" wire:model.blur="generationCount"
                                class="mt-2 w-full rounded-2xl border-sky-200 bg-white/90 px-4 py-3 text-slate-900 focus:border-sky-500 focus:ring-sky-300">
                            @error('generationCount')
                                <p class="mt-2 text-sm text-rose-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div>
                            ...
                            <input type="number" min="5" max="600"
                                wire:model.blur="generationResponseTimeSeconds"
                                class="mt-2 w-full rounded-2xl border-sky-200 bg-white/90 px-4 py-3 text-slate-900 focus:border-sky-500 focus:ring-sky-300">
                            @error('generationResponseTimeSeconds')
                                <p class="mt-2 text-sm text-rose-600">{{ $message }}</p>
                            @enderror
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap gap-3">
                        ...
                        <button type="button" wire:click="addQuestion"
                            class="rounded-2xl border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-800 transition hover:border-slate-400 hover:bg-slate-50">
                            Pridať jednu otázku
                        </button>
                    </div>
                </form>
        ...
        <section class="rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-slate-200/80">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500">Váha 0 znamená, že zariadenie sa do výsledku nezapočíta.
                        Ostatné čísla sa pripočítajú k zvolenej odpovedi.</p>
                </div>
                <div class="flex flex-wrap gap-3">
                    ...
                    <div
                        class="rounded-full bg-slate-900 px-4 py-3 text-xs font-semibold uppercase tracking-[0.25em] text-white">
            ...
            <div class="mt-6 grid gap-5 lg:grid-cols-2">
                <form wire:submit="assignBulkDeviceWeights"
                    ...
                    <div class="mt-4 grid gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <input type="number" min="0" step="0.01" wire:model.blur="bulkDeviceWeight"
                                class="mt-2 w-full rounded-2xl border-slate-300 px-4 py-3 text-slate-900 focus:border-sky-500 focus:ring-sky-300">
                            @error('bulkDeviceWeight')
                                <p class="mt-2 text-sm text-rose-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div>
                            <label class="text-sm font-medium text-slate-700">Počet zariadení od 1</label>
                            <input type="number" min="1" wire:model.blur="bulkDeviceCount"
                                class="mt-2 w-full rounded-2xl border-slate-300 px-4 py-3 text-slate-900 focus:border-sky-500 focus:ring-sky-300">
                            @error('bulkDeviceCount')
                                <p class="mt-2 text-sm text-rose-600">{{ $message }}</p>
                            @enderror
                        </div>
                    </div>
                    <button type="submit"
                        class="mt-4 rounded-2xl bg-sky-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-sky-700">
                        Priradiť váhu zariadeniam
                    </button>
                </form>

                <form wire:submit="importDeviceWeights"
                    class="rounded-[1.5rem] border border-slate-200 bg-slate-50 p-5">
                        ...
                        <thead class="sticky top-0 bg-slate-50">
                            <tr>
                                <th
                                    ...
                                    Zariadenie</th>
                                <th
                                    class="w-44 px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">
                                    Počet hlasov</th>
                            </tr>
                        </thead>
                                ...
                                <tr wire:key="voting-device-weight-{{ $deviceRow['id'] }}">
                                    ...
                                    <td class="px-4 py-3">
                                        <input type="number" min="0" step="0.01"
                                            wire:model.blur="deviceWeightRows.{{ $index }}.weight"
                                            class="w-full rounded-xl border-slate-300 px-3 py-2 text-right text-sm text-slate-900 focus:border-sky-500 focus:ring-sky-300">
                                    </td>
                                </tr>
                            @empty
                                <tr>
        ...
        <section class="rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-slate-200/80">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500">Každá otázka má vlastný čas odpovede. Texty môžeš meniť
                        priebežne pred spustením.</p>
                </div>
                <div
                        ...
                        <div class="grid gap-5 xl:grid-cols-[0.8fr,2fr,0.8fr,auto] xl:items-start">
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <textarea wire:model.blur="questionRows.{{ $index }}.text" rows="3"
                                        class="mt-2 w-full rounded-2xl border-slate-300 px-4 py-3 text-slate-900 focus:border-sky-500 focus:ring-sky-300"></textarea>
                                </div>
                            </div>
                            ...
                            <div class="flex gap-3 xl:justify-end">
                                <button type="button" wire:click="saveQuestion({{ $question['id'] }})"
                                ...
                                <button type="button" wire:click="deleteQuestion({{ $question['id'] }})"
                                    wire:confirm="Naozaj chceš vymazať túto otázku?"
