<div wire:poll.500ms class="relative h-screen overflow-hidden bg-white text-slate-950">
    @if ($contest)
        <div class="flex h-full flex-col px-10 py-5"><header class="flex items-center gap-6"><div class="flex h-14 w-48 items-center justify-center">@if($voting->logo_path)<img src="{{ route('votings.logo', $voting) }}" alt="Logo hlasovania" class="h-full w-full object-contain">@endif</div><div><h1 class="text-4xl font-semibold">{{ $voting->title ?: $voting->name }}</h1><p class="mt-2 text-3xl font-semibold text-emerald-700">{{ $contest->name }}</p></div></header><main class="mt-6 grid min-h-0 flex-1 grid-cols-[minmax(0,1fr)_18rem] gap-8"><div><p class="text-2xl text-slate-500">Kandidátka</p><div class="mt-4 grid grid-cols-2 gap-4">@foreach($contest->candidates as $candidate)<div class="rounded-2xl bg-slate-100 px-6 py-4 text-3xl">{{ $candidate->first_name }} {{ $candidate->last_name }}</div>@endforeach</div></div><aside class="flex flex-col justify-between border-l pl-6 text-right"><div class="text-2xl">Mandátov: <strong>{{ $contest->seat_count }}</strong></div><div class="text-xl text-slate-500">Pripravené na hlasovanie</div></aside></main></div>
    @elseif ($round)
        <div class="flex h-full flex-col px-10 py-5"><header class="flex items-center gap-6"><div class="flex h-14 w-48 items-center justify-center">@if($voting->logo_path)<img src="{{ route('votings.logo', $voting) }}" alt="Logo hlasovania" class="h-full w-full object-contain">@endif</div><div><h1 class="text-4xl font-semibold">{{ $voting->title ?: $voting->name }}</h1><p class="mt-2 text-3xl font-semibold text-emerald-700">{{ $round->contest->name }} · kolo {{ $round->round_number }}</p></div></header><main class="mt-6 grid min-h-0 flex-1 grid-cols-[minmax(0,1fr)_18rem] gap-8"><div><div class="grid grid-cols-[5rem_1fr_12rem_10rem] gap-4 border-b pb-3 text-xl font-semibold text-slate-500"><span>Por.</span><span>Kandidát</span><span>Hlasy</span><span>Stav</span></div>@foreach($roundResults['candidates'] as $index => $candidate)<div class="grid grid-cols-[5rem_1fr_12rem_10rem] gap-4 border-b py-5 text-3xl {{ $candidate['elected'] ? 'bg-emerald-100' : '' }}"><span>{{ $index + 1 }}</span><span>{{ $candidate['first_name'] }} {{ $candidate['last_name'] }}</span><strong>{{ $candidate['weighted_total'] }}</strong><span>{{ $candidate['elected'] ? 'Zvolený' : '' }}</span></div>@endforeach</div><aside class="flex flex-col justify-between border-l pl-6 text-right"><div class="space-y-3 text-2xl"><p>{{ $round->status === 'live' ? 'Prebieha hlasovanie' : 'Výsledok kola' }}</p><p>Mandátov: <strong>{{ $round->contest->seat_count }}</strong></p><p>Väčšina: <strong>{{ $roundResults['majority_threshold'] }}</strong></p></div><div class="text-xl text-slate-500">Prijaté vážené hlasy: {{ $roundResults['total_weight'] }}</div></aside></main></div>
    @elseif ($admission)
        @php
